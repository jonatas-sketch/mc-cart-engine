...
/* Flags do bloco multi-pixel (ver 'Multi-pixel da Meta' abaixo). theme-drawer: nomeia o pixel das campanhas (#461) e garante o SDK sem depender do app da Meta (#462). */
var MC_META_NOMEAR_DESTINO = true;
var MC_META_CARREGAR_SDK = true;
...
function sendCAPI(eventName, data, cur, eventId){
  try {
    ...
    fetch(C.capiEndpoint, {
      ...
      if (res.success) {
        console.log('[MC Page CAPI] ' + metaEvent + ' sent ok (events_received: ' + res.events_received + ')');
        if (res.pixels_failed) console.warn('[MC Page CAPI] ' + metaEvent + ': ' + res.pixels_failed + ' pixel(s) recusaram — confira o token no painel (Tracking → Meta → testar)');
