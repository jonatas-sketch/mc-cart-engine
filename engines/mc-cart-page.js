...
async function ensureMemberAssigned(){
  ...
  if (!assignedMember) {
    // 2026-08-23: loader used to emit members:[] on a 3s DB timeout and
    // cache that JS for 5 min. theme-drawer cannot checkout with an empty
    // pool. Last resort — single-checkout credentials already on config
    // (plus optional C.legacyProductMappings when the loader synthesized).
...
function maybeRedirectToCf(vitrineCart){
  ...
  window.location.href = cfBuildBridgeUrl(picked.target, picked.cfLines, passthrough);
  return true;
}
